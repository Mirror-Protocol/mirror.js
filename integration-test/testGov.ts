...
export async function testGov(mirror: Mirror) {
  ...
  const initialVotingRewards: number = parseInt(initialState.pending_voting_rewards);

  const balanceRes = await mirror.mirrorToken.getBalance(mirror.gov.contractAddress); 
  const initialBalance: number = parseInt(balanceRes.balance);

  console.log('Stake MIR tokens');
  const stakeRes = await execute(
    test1,
    mirror.gov.stakeVotingTokens(
  ...
  if (initialBalance > 0) {
    addedShare = Math.trunc(1000100 * initialShare / (initialBalance - initialDeposit - initialVotingRewards));
  }
  ...
  assert(state.pending_voting_rewards == (initialVotingRewards + Math.trunc(1000000 * voterWeight)).toString())
  
  console.log('Snapshot poll - expect error');
  await execute(
    test1,
    mirror.gov.snapshotPoll(parseInt(pollId))
  ).catch((error) => {
    assert(error.response.data.error.includes('Cannot snapshot at this height'));
  });

  console.log('End poll - expect error');
  await execute(
    test1,
    mirror.gov.endPoll(parseInt(pollId))
  ).catch((error) => {
  ...
  await execute(
    test1,
    mirror.gov.withdrawVotingTokens(100)
  );

  console.log('Query staker');
  let stakerRes = await mirror.gov.getStaker(test1.key.accAddress);
  
  if (stakerRes.pending_voting_rewards != '0') {
    console.log('Withdraw voting rewards');
    await execute(
      test1,
      mirror.gov.withdrawVotingRewards()
    );
  } else {
