...
export async function testCollateralOracle(mirror: Mirror) {
  ...
  const configResponse = await mirror.collateralOracle.getConfig();
  ...
  await execute(
    test1,
    mirror.collateralOracle.revokeCollateralAsset({
      native_token: { denom: randomCollateralDenom }
    })
  );

  const collateralPriceRes = await mirror.collateralOracle.getCollateralPrice(
  ...
  await execute(
    test1,
    mirror.collateralOracle.updateCollateralMultiplier(
      { native_token: { denom: randomCollateralDenom } },
      1.0
    )
  );

  const collateralInfoRes = await mirror.collateralOracle.getCollateralAssetInfo(
    randomCollateralDenom
  );
  ...
  await execute(
    test1,
    mirror.collateralOracle.updateCollateralPriceSource(
      { native_token: { denom: randomCollateralDenom } },
  ...
  const priceRes2 = await mirror.collateralOracle.getCollateralPrice(randomCollateralDenom);
  ...
  console.log('Query collateral price');
  await mirror.collateralOracle.getCollateralPrice(randomCollateralDenom).catch(console.log); // will fail if pool is empty
}
